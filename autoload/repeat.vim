...
if exists("g:loaded_repeat") || &compatible || v:version < 700
	finish
endif
		...
		if g:repeat_tick == b:changedtick
			...
			if g:repeat_reg[0] ==# g:repeat_sequence && !empty(g:repeat_reg[1])
				...
				if regname ==# '='
					...
					" is what we want.
					let register = '"=' . getreg('=', 1) . "\<CR>"
				else
					let register = '"' . regname
				endif
			endif
			...
			elseif v:version <= 703
				call feedkeys(register . count, 'n')
				call feedkeys(sequence, '')
			else
				call feedkeys(sequence, 'i')
				call feedkeys(register . count, 'ni')
			endif
		else
			if ((v:version == 703 && has('patch100')) || (v:version == 704 && !has('patch601')))
...
nnoremap <silent> <Plug>(RepeatDot)      <Cmd>execute repeat#run(v:count)<CR>
nnoremap <silent> <Plug>(RepeatUndo)     <Cmd>call repeat#wrap('u',v:count)<CR>
nnoremap <silent> <Plug>(RepeatUndoLine) <Cmd>call repeat#wrap('U',v:count)<CR>
nnoremap <silent> <Plug>(RepeatRedo)     <Cmd>call repeat#wrap("\<Lt>C-R>",v:count)<CR>
